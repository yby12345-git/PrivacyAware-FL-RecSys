...
def add_dp_noise(state_dict, noise_std):
    noisy = copy.deepcopy(state_dict)
    for k in noisy.keys():
        if torch.is_floating_point(noisy[k]):
            noisy[k] = noisy[k] + torch.normal(
                mean=0.0,
                std=noise_std,
                size=noisy[k].shape,
                device=noisy[k].device
            )
    return noisy

def local_train(model, df, device, batch_size, lr, local_epochs, clip_norm, noise_std):
    ...
    state = add_dp_noise(state, noise_std)
...
def main():
    ...
    print("DP noise_std:", args.noise_std, "epsilon:", args.epsilon)
    ...
    name = f"dp_fedavg_{args.dataset}_eps{args.epsilon}"
    out.to_csv(f"outputs/tables/{name}.csv", index=False)
    torch.save(global_model.state_dict(), f"models/{name}.pt")

    print("Saved:", f"outputs/tables/{name}.csv")
